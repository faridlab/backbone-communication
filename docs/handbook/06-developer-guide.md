...
metaphor migration generate Invoice communication
metaphor migration run
...
(`communication` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed communication          # run Rust seeders
metaphor migration generate-seeds communication  # emit SQL seed files
```
