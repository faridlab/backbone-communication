...
//!
//! Tenancy (ADR-0029): the inbound path stages the routing event's outbox mirror (company-keyed, as the
//! framework outbox remains), so the probe binds a company-anchored org request scope around the flow.
...
async fn cseam1_inbound_message_opens_real_support_ticket() {
    ...
    let m = InboundMessage {
        ...
        external_id: format!("m-{}", Uuid::new_v4()),
        address_from: Some("+628123".into()),
        address_to: Some("+628999".into()),
        body: "Barang saya rusak, tolong bantu".into(),
        party_id: Some(party),
        external_ref: Some("wa-conv-9".into()),
        subject_type: None,
        subject_id: None,
    };
    let issue_id = with_company_scope(&pool, Uuid::new_v4(), async {
        ...
        support
            ...
            .await
            .expect("real support raises the ticket")
    })
    .await;
    ...
    assert_eq!(
        desc.as_deref(),
        Some("Barang saya rusak, tolong bantu"),
