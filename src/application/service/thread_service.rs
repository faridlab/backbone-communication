//! Application service for Thread entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `thread_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateThreadDto, UpdateThreadDto};
use crate::domain::entity::Thread;
use crate::infrastructure::persistence::ThreadRepository;

/// Application service for Thread entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `thread_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct ThreadServiceCustom {
///     inner: Arc<ThreadService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type ThreadService = GenericCrudService<
    Thread,
    CreateThreadDto,
    UpdateThreadDto,
    ThreadRepository,
>;

// <<< CUSTOM
// END CUSTOM
